...
const fetchHour24 = async () => {
  ...
  const res = await fetch(BASE_URL + `/pvuv?date_level=hour&end_time=${Date.now()}`).then((res) => res.json());
  const data = res?.data || [];
...
const fetchDay30 = async () => {
  ...
  const res = await fetch(BASE_URL + `/pvuv?date_level=day&end_time=${Date.now()}`).then((res) => res.json());
  const data = res?.data || [];
  data.forEach((item: any) => {
